        ...
        ((_, firstLine) : rest) ->
            ...
            in case cpoHeaderOption options of
                WithHeader ->
                    ...
                    in unsafeMkCsv (Just headerFields) <$> parseColumns expectedLength rest
                WithoutHeader -> unsafeMkCsv Nothing <$> parseColumns expectedLength lines
        where 
            lines :: [(Int, T.Text)]
            lines =
                L.filter (\(_,t) -> not $ T.null t) 
                         $ L.zip [1 .. ]
                         $ T.split (== (sepLineSeparator $ cpoSeparators options)) raw

            splitFields :: T.Text -> [T.Text]
            splitFields = L.map T.strip . T.splitOn separator
                where 
                    separator :: T.Text
                    separator = T.singleton $ sepFieldSeparator (cpoSeparators options)

            parseColumns :: Int -> [(Int, T.Text)] -> Either String [[DataField]]
            parseColumns expectedLength lines =
                ...
                where
                    parseRow :: Either String [[T.Text]] -> (Int, T.Text) -> Either String [[T.Text]]
...
parseWithHeader = parseCsv (defaultOptions {cpoHeaderOption = WithHeader})
